...
public class LevelModel<T>
{
	...
	public LevelModel(Corpus<T> corpus)
	{
		add(corpus);
	}
	...
	public double relevance(T token)
	{
		...
		double mean = (2.0 * n - 1)/(2.0 * n + 2);
		double stdv = 1 / ( sqrt(n) * (1.0 + 2.8 * pow(n, -0.865)) );
	
		return (relevanceSigNorm(token) - mean)/stdv;
	}
	
	/**
	 * @return A list of all the encountered tokens. The list is not connected 
